...
public class OrderedActivityRestServiceImpl implements OrderedActivityRestService {
    ...
    public OrderedQuantity addActivityToPlan(UUID planId, CreateOrderedActivityRequestDTO request) {
        ...
        // 5. Calculate current total ordered quantity in THIS PLAN only (exclude
        // soft-deleted)
        ...
        // 6. Validate: Total ordered quantity in THIS PLAN must not exceed Package
        // quota
        if (currentTotalOrderedInPlan + request.getOrderedQuantity() > tourPackage.getQuota()) {
            throw new RuntimeException(
                    String.format(
                            "Cannot add activity. Total ordered quantity in this plan (%d + %d = %d) would exceed package quota (%d)",
        ...
        if (plan.getOrderedQuantities() == null) {
            plan.setOrderedQuantities(new java.util.ArrayList<>());
        }
        Long totalPrice = plan.getOrderedQuantities().stream()
                .filter(java.util.Objects::nonNull)
                .peek(oq -> System.out.println("DEBUG: OQ ID=" + oq.getId()))
                .filter(oq -> !Boolean.TRUE.equals(oq.getIsDeleted()))
                .peek(oq -> System.out
                        .println("DEBUG: OQ not deleted. Quota=" + oq.getOrderedQuota() + ", Price=" + oq.getPrice()))
                .mapToLong(oq -> {
                    long quota = oq.getOrderedQuota();
                    long price = oq.getPrice() != null ? oq.getPrice() : 0;
                    return quota * price;
                })
                .sum();
        ...
        // Check if all ordered quantities can be fulfilled (ordered <= activity
        // capacity)
        boolean allActivitiesCanFulfill = plan.getOrderedQuantities().stream()
                ...
                .allMatch(oq -> {
                    ...
                    if (!canFulfill) {
                        System.out.println("   ⚠️ Activity '" + act.getActivityName() + "' cannot fulfill: ordered="
                                + oq.getOrderedQuota() + ", capacity=" + act.getCapacity());
                    }
                    return canFulfill;
                });
        ...
        if (newTotalOrderedInPlan >= tourPackage.getQuota() && allActivitiesCanFulfill) {
            ...
            System.out.println(
                    "   - Total ordered: " + newTotalOrderedInPlan + " >= Package quota: " + tourPackage.getQuota());
            ...
            if (newTotalOrderedInPlan < tourPackage.getQuota()) {
                System.out.println("📋 Plan status: Unfulfilled (ordered: " + newTotalOrderedInPlan + " < quota: "
                        + tourPackage.getQuota() + ")");
            } else {
    ...
    public OrderedQuantity updateOrderedActivity(UUID orderedActivityId, int newQuantity) {
        ...
        // 3. Calculate current total in THIS PLAN (excluding this ordered quantity and
        // soft-deleted)
        ...
        if (currentTotalExcludingThis + newQuantity > tourPackage.getQuota()) {
            throw new RuntimeException(
                    String.format(
                            "Cannot update. Total ordered in this plan (%d + %d = %d) would exceed package quota (%d)",
