...
class ActivityRestControllerTest {
        ...
        /**
         * Helper method yang membungkus AuthenticatedUser ke dalam objek
         * Authentication.
        ...
        void getAllActivities_noFilters_shouldReturnAllActiveSortedByDate() throws Exception {
                // Arrange
                Activity a1 = mockActivity.toBuilder().id("ACT-A").startDate(LocalDateTime.now().plusDays(1))
                                .isDeleted(false).activityType("Flight").build();
                Activity a2 = mockActivity.toBuilder().id("ACT-B").startDate(LocalDateTime.now().plusDays(3))
                                .isDeleted(false).activityType("Accommodation").build();
                Activity a3_deleted = mockActivity.toBuilder().id("ACT-C").startDate(LocalDateTime.now().plusDays(2))
                                .isDeleted(true).activityType("Flight").build();
        ...
        void getAllActivities_filterAllNullsAndEmptyStrings_shouldReturnAll() throws Exception {
                ...
                Activity a1 = mockActivity.toBuilder().id("ACT-A").activityType(null).startLocation(null)
                                .endLocation("").activityItem(null).build();
        ...
        void createActivity_asCustomer_accessDenied() throws Exception {
                ...
                mockMvc.perform(post("/api/activities")
                                .with(SecurityMockMvcRequestPostProcessors
                                                .authentication(createAuthentication(mockCustomerUser)))
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(objectMapper.writeValueAsString(request)))
        ...
        void updateActivity_vendorNotOwner_shouldReturn403() throws Exception {
                UpdateActivityRequestDTO request = new UpdateActivityRequestDTO("Updated Name", null, null, null, null,
                                null, null, null);
        ...
        void updateActivity_hasFulfilledOrderedActivities_shouldReturn400() throws Exception {
                ...
                Plan plan = Plan.builder().tourPackage(fulfilledPackage).build();
                ...
                mockMvc.perform(put("/api/activities/{id}", ACTIVITY_ID)
                                ...
                                .andExpect(jsonPath("$.message")
                                                .value("Cannot update activity that has fulfilled orderedActivities"));
        }
        ...
        void deleteActivity_hasUnfulfilledOrderedActivities_shouldReturn400() throws Exception {
                ...
                Plan plan = Plan.builder().tourPackage(pendingPackage).build();
                ...
                mockMvc.perform(delete("/api/activities/{id}", ACTIVITY_ID)
                                ...
                                .andExpect(jsonPath("$.message").value(
                                                "Cannot delete activity that has unfulfilled orderedActivities. Only activities with no orders or only fulfilled orders can be deleted."));
        }

        @Test
        void deleteActivity_alreadyDeleted_shouldReturn400() throws Exception {
                Activity deletedActivity = mockActivity.toBuilder().vendorId(VENDOR_ID_FROM_JWT).isDeleted(true)
                                .build();
                when(activityRepository.findById(ACTIVITY_ID)).thenReturn(Optional.of(deletedActivity));
        ...
        void deleteActivity_repositoryThrowsException_shouldReturn500() throws Exception {
                Activity activeActivity = mockActivity.toBuilder().vendorId(VENDOR_ID_FROM_JWT).isDeleted(false)
                                .orderedQuantities(Collections.emptyList()).build();
