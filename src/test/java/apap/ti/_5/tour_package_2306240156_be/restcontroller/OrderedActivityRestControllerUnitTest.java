...
import apap.ti._5.tour_package_2306240156_be.security.WebSecurityConfig;
...
@WebMvcTest(OrderedActivityRestController.class)
@Import({ WebSecurityConfig.class, JwtTokenFilter.class, ApiKeyFilter.class })
class OrderedActivityRestControllerUnitTest {
    ...
    void testGetEligibleActivities_Success() throws Exception {
        ...
        when(activityRepository.findEligibleActivitiesForPlan(
                ...
                plan.getEndLocation())).thenReturn(Arrays.asList(activity));

        mockMvc.perform(get("/api/ordered-activities/eligible")
                .param("planId", planId.toString())
                .with(authentication(new TestingAuthenticationToken(createAuthenticatedUser(), null, "ROLE_Customer")))
    ...
    void testCreateOrderedActivity_ExceedsActivityCapacity() throws Exception {
        ...
        mockMvc.perform(post("/api/ordered-activities/create")
                ...
                .content(objectMapper.writeValueAsString(createRequest)))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.message").value(containsString("exceeds available capacity")));
    ...
    void testCreateOrderedActivity_MakesPlanFulfilled() throws Exception {
        ...
        newOq.setId(orderedActivityId);
        newOq.setPlan(plan);
        newOq.setOrderedQuota(10);
        newOq.setIsDeleted(false);

        when(planRepository.findById(planId)).thenReturn(Optional.of(plan));
        when(activityRepository.findById(activityId)).thenReturn(Optional.of(activity));

        // Mock findAll to return updated list on second call
        when(orderedQuantityRepository.findAll())
                .thenReturn(Arrays.asList(existingOq)) // For validation
                .thenReturn(Arrays.asList(existingOq, newOq)); // For status update
        ...
        verify(planRepository, times(1))
                .save(argThat(p -> p.getId().equals(planId) && "Fulfilled".equals(p.getStatus())));
    }
    ...
    void testDeleteOrderedActivity_Success() throws Exception {
        ...
        verify(orderedQuantityRepository, times(1)).findById(orderedActivityId);
        verify(orderedQuantityRepository, times(1))
                .save(argThat(oq -> oq.getId().equals(orderedActivityId) && oq.getIsDeleted()));
    }
    ...
    void testCreateOrderedActivity_PackageNotFound() throws Exception {
        ...
        mockMvc.perform(post("/api/ordered-activities/create")
                .param("planId", planId.toString())
                .with(authentication(new TestingAuthenticationToken(createAdminUser(), null, "ROLE_Superadmin")))
                .content(objectMapper.writeValueAsString(createRequest))
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isBadRequest())
    ...
    void testCreateOrderedActivity_UpdateExistingActivity_Success() throws Exception {
        ...
        verify(orderedQuantityRepository, times(1))
                .save(argThat(oq -> oq.getOrderedQuota() == 15 && oq.getId().equals(existing.getId())));
    }
